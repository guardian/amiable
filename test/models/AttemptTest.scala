...
class AttemptTest
  ...
  "traverse" - {
    "returns the first failure" in {
      ...
      val errors = Attempt
        .traverse(List(1, 2, 3, 4, 5))(failOnFourAndSix)
        .awaitEither
        .left
        .value
    ...
    "returns the successful result if there were no failures" in {
      Attempt
        .traverse(List(1, 2, 3, 4))(Right)
        .awaitEither
        .right
        .value shouldEqual List(1, 2, 3, 4)
    }
  }
  ...
  def expectedFailure[A](message: String): Attempt[A] =
    Left[A](AMIableError(message, "this will fail", 500))
}
