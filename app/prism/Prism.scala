...
object Prism {
  ...
  def getInstances(stackStageApp: SSAA)(implicit
      config: AMIableConfig,
      ec: ExecutionContext
  ...
  def imageUsage(image: AMI)(implicit
      config: AMIableConfig,
      ec: ExecutionContext
  ): Attempt[List[Instance]] = {
  ...
  def instancesWithAmis(stackStageApp: SSAA)(implicit
      config: AMIableConfig,
      ec: ExecutionContext
  ...
  def launchConfigUsage(image: AMI)(implicit
      config: AMIableConfig,
      ec: ExecutionContext
  ): Attempt[List[LaunchConfiguration]] = {
    val url = imageLaunchConfigUrl(image.imageId, config.prismUrl)
    for {
      response <- Http.response(
        config.wsClient.url(url).get(),
        "Unable to fetch launch configurations",
        url
      )
      jsons <- launchConfigurationResponseJson(response)
      launchConfigurations <- Attempt.sequence(
        jsons.map(extractLaunchConfiguration)
      )
