...
class ScheduledNotificationRunner @Inject() (
...
object ScheduledNotificationRunner extends Logging {
  def ownerForInstance(i: Instance, owners: Owners): Owner = {
    owners.owners
      .find(_.hasSSA(SSAA(i.stack, i.stage, i.app.headOption)))
      .orElse(
        owners.owners.find(_.hasSSA(SSAA(i.stack, app = i.app.headOption)))
      )
